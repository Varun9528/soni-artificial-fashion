// Test script to verify the full cart flow including removal functionality
const testFullCartFlow = async () => {
  try {
    console.log('Testing full cart flow...');
    ...
    const itemExists = getCartAfterRemovalData.items.some(item => item.productId === 'test-product-1');
    if (itemExists) {
